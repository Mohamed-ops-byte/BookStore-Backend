# Bookstore Backend API - دليل الاستخدام
...
1. **قاعدة البيانات**: تأكد من تثبيت MySQL أو تفعيل SQLite PHP Extension
2. **CORS**: للسماح بـ Requests من React Frontend، تأكد من إعداد CORS في `config/cors.php`
3. **الصور**: الصور تُحفظ في `storage/app/public/books/` ويجب تشغيل `php artisan storage:link`
4. **Environment**: تأكد من إعدادات `.env` الصحيحة
...
- `app/Http/Controllers/BookController.php` - إدارة الكتب
